...
<!--          </el-button>-->
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%" size="mini">
      ...
      <el-table-column align="center" label="Thao Tác" width="180">
        <template slot-scope="scope">
          <router-link :to = "{ name: 'EditTerm', params: { id: scope.row.id }}" v-if="includeRoles(userRoles, [allRoles.admin, allRoles.exams_maker], false)">
            <el-button type="primary" size="mini" icon="el-icon-edit" title="Chỉnh Sửa">
            </el-button>
          </router-link>
          <router-link :to = "{ name: 'TermDetail', params: { id: scope.row.id }}" v-if="includeRoles(userRoles, [allRoles.admin, allRoles.exams_maker], false)">
