...
export default {
  ...
  methods: {
    ...
    listChapters() {
      this.loading = true;
      subjectResource.chapters(this.subject.slug).then(response => {
        const { data } = response;
    ...
    createChapter(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          subjectResource.storeChapter(this.newChapter, this.subject.slug).then(response => {
    ...
    confirmEdit(row, formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.loading = true;
          subjectResource.updateChapter(row, this.subject.slug, row.id).then(response => {
            if (response.error) {
              this.$message({
