              ...
              <div class="col-md-9">
                <div class="row">
                  <div class="col-md-6" v-for="cht in allChapters" :key="cht.id">
                    <el-form-item :label="cht.name + ' (' + 'Max: ' + cht.question_num +')'" prop="format" :label-position="'right'">
  ...
  methods: {
    ...
    updateExamFormat(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          subjectResource.updateExamFormat(this.newExamFormat, this.subject.id, this.examFormat.id).then(response => {
            if (response.error) {
              this.$message({
